...
class SimpleGraphIndex:
    ...
    def remove_node(self, node_id: str) -> None:
        ...
        for source, _ in self.reverse_adjacency[node_id]:
            ...
            self.adjacency[source] = [
                (n, w) for n, w in self.adjacency[source] if n != node_id
            ]
    ...
    def store(self, directory: Path) -> dict[str, Any]:
        ...
        edges_data = {
            node_id: [(n, w) for n, w in neighbors]
    ...
    def load(cls, name: str, directory: Path) -> "SimpleGraphIndex":
        ...
        if edges_file.exists():
            ...
            for from_node, neighbors in edges_data.items():
                index.adjacency[from_node] = [(n, w) for n, w in neighbors]
...
class GraphMemoryCollection(BaseMemoryCollection):
    ...
    def store(self, data_dir: str | None = None) -> dict[str, Any]:
        ...
        # Store text storage
        text_data = {text_id: self.text_storage.get(text_id) for text_id in self.text_storage.get_all_ids()}
        ...
        metadata_data = {
            "store": {mid: self.metadata_storage.get(mid) for mid in metadata_ids}
        }
