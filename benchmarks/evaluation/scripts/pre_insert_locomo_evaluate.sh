#!/bin/bash
# ============================================================================
# pre_insert_locomo_evaluate.sh
#
# LoCoMo PreInsert实验评估脚本
# 自动发现并分析所有PreInsert_*目录
# ============================================================================
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "示例:"
            echo "  $0                                          # 分析所有PreInsert_*目录"
            echo "  $0 --validate-only                          # 仅验证"
            echo "  $0 --path PreInsert_MemoryOS_forgetting_curve  # 分析指定目录"
            exit 0
            ;;
        *)
            echo "未知参数: $1"
...
# 构建命令 - 指定独立的输出目录
OUTPUT_DIR=".sage/benchmarks/benchmark_memory/locomo/output/round_analysis_pre_insert"
