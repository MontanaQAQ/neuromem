#!/bin/bash
# Run MemoryOS Longmemeval PostRetrieval Experiment - multi_query
# Usage: bash script/context_integration_mechanism/run_memoryos_longmemeval_multi_query.sh
...
CONFIG_FILE="$SCRIPT_DIR/../../config/context_integration_mechanism/MemoryOS_longmemeval_multi_query_post_retrieval_pipeline.yaml"

TASK_IDS=("group-1" "group-2" "group-3")

DATASET="longmemeval"
MEMORY_NAME="PostRetrieval_MemoryOS_multi_query"
LOG_BASE_DIR="$PROJECT_ROOT/.sage/output/benchmarks/benchmark_memory/$DATASET/$MEMORY_NAME"
mkdir -p "$LOG_BASE_DIR"

echo "========================================================================"
echo "MemoryOS Longmemeval PostRetrieval Experiment - multi_query"
echo "========================================================================"
