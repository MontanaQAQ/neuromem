#!/bin/bash
# 运行 Llama-3.1-8B 补充实验 - Feature Summary Vector
# 使用方法: bash script/additional_experiments_ds_llama/run_llama_feature_summary_vector.sh

set -e  # 遇到错误立即退出

# 获取脚本所在目录的绝对路径
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# 获取项目根目录 (从 script/additional_experiments_ds_llama/ 向上 4 层到 neuromem/)
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# Python 脚本的相对路径
PYTHON_SCRIPT="$SCRIPT_DIR/../../memory_test_pipeline.py"
# 配置文件路径
CONFIG_FILE="$SCRIPT_DIR/../../config/additional_experiments_ds_llama/llama_feature_summary_vector_pipeline.yaml"
...
# 创建日志目录结构
DATASET="locomo"
MEMORY_NAME="Llama_DataStructure_feature_summary_vectorstore"
LOG_BASE_DIR="$PROJECT_ROOT/.sage/output/benchmarks/benchmark_memory/$DATASET/$MEMORY_NAME"
mkdir -p "$LOG_BASE_DIR"

echo "========================================================================"
echo "Llama-3.1-8B 补充实验 - Feature Summary Vector"
echo "========================================================================"
echo ""
...
echo "========================================================================"
echo "🎉 所有任务执行完毕 - Llama_DataStructure_feature_summary_vectorstore"
echo "========================================================================"
