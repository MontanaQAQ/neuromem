#!/bin/bash
# 运行 Memory Data Structure 实验 - Feature Summary Vectorstore Combination
# 使用方法: bash script/memory_data_structure/run_DataStructure_feature_summary_vectorstore.sh

set -e  # 遇到错误立即退出

# 获取脚本所在目录的绝对路径
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# 获取项目根目录 (从 script/memory_data_structure/ 向上 4 层到 neuromem/)
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
# Python 脚本的相对路径
PYTHON_SCRIPT="$SCRIPT_DIR/../../memory_test_pipeline.py"
# 配置文件路径
CONFIG_FILE="$SCRIPT_DIR/../../config/memory_data_structure/locomo_feature_summary_vector_pipeline.yaml"
...
# 创建日志目录结构
DATASET="locomo"
MEMORY_NAME="DataStructure_feature_summary_vectorstore"
LOG_BASE_DIR="$PROJECT_ROOT/.sage/output/benchmarks/benchmark_memory/$DATASET/$MEMORY_NAME"
mkdir -p "$LOG_BASE_DIR"

echo "========================================================================"
echo "Memory Data Structure 实验 - Feature Summary Vectorstore Combination"
echo "========================================================================"
echo ""
...
echo "========================================================================"
echo "🎉 所有任务执行完毕 - DataStructure_feature_summary_vectorstore"
